...
Login::Login(QWidget *parent)
    : QDialog(parent), ui(new Ui::login), mainWindow(new MainWindow(nullptr, &db)),
      setting(new Setting) {
  // 默认登陆设置(图标，标题，背景之类的)
...

  {
    ...
    bool build = setting->getFromFile(dir.remove("cmake-build-release") + "settings.json");
    if(!build) {
      build = setting->getFromFile(dir.remove("build") + "settings.json");
      if(!build) {
        qDebug() << "Settings 文件打开失败!";
      }
    }
  }
  login_init();
